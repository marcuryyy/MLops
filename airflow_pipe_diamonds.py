...
def clear_data(path2df='data.csv'):
...
dag_diamonds = DAG(
   ...
   # concurrency=4, # Убрали в airflow3
    #schedule_interval=timedelta(minutes=5), # убрали в airflow3
    schedule="@hourly",
    max_active_runs=1,
    catchup=False,
)
download_task = PythonOperator(python_callable=download_dataset, task_id="download_diamons_dataset", dag=dag_diamonds)
clear_task = PythonOperator(python_callable=clear_data, task_id="clear_dataset", dag=dag_diamonds)
train_task = PythonOperator(python_callable=train, task_id="train_model", dag=dag_diamonds)
